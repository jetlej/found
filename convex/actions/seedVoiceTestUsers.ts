...
async function generateBasics(persona: Persona): Promise<BasicsResult> {
  const prompt = BASICS_PROMPT.replace('{name}', persona.name)
...
export const seedSingleVoiceTestUser = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (answers.length !== VOICE_QUESTIONS.length) {
      throw new Error(`Expected ${VOICE_QUESTIONS.length} answers, got ${answers.length}`);
    }
    ...
    await ctx.scheduler.runAfter(0, internal.actions.seedTestPhotos.seedPhotosForPersona, {
      userId,
      name: persona.name,
      gender: persona.gender,
      description: persona.description,
...
export const backfillSingleUser = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const profile = await ctx.runQuery(internal.userProfiles.getByUserInternal, {
      userId: args.userId,
    });
