...
export default function SmokingScreen() {
  ...
  useEffect(() => {
    if (currentUser && !hasLoadedData) {
      if (currentUser.smoking) setSelected(currentUser.smoking);
      if (currentUser.smokingVisible !== undefined) setVisible(currentUser.smokingVisible);
  ...
  const handleContinue = async () => {
    ...
    setLoading(true);
    try {
      await updateBasics({ clerkId: userId, smoking: selected, smokingVisible: visible });
      if (!isEditing) await setOnboardingStep({ clerkId: userId, step: "marijuana" });
      goToNextStep(router, "smoking", isEditing);
    } catch { setLoading(false); }
  };

  return (
    <OnboardingScreen question="Do you smoke?" canProceed={!!selected} loading={loading} onNext={handleContinue} onBack={isEditing ? () => router.back() : undefined}>
