...
export default function DrinkingScreen() {
  ...
  useEffect(() => {
    if (currentUser && !hasLoadedData) {
      if (currentUser.drinking) setSelected(currentUser.drinking);
      if (currentUser.drinkingVisible !== undefined) setVisible(currentUser.drinkingVisible);
  ...
  const handleContinue = async () => {
    ...
    setLoading(true);
    try {
      await updateBasics({ clerkId: userId, drinking: selected, drinkingVisible: visible });
      if (!isEditing) await setOnboardingStep({ clerkId: userId, step: "smoking" });
      goToNextStep(router, "drinking", isEditing);
    } catch { setLoading(false); }
  };

  return (
    <OnboardingScreen question="Do you drink?" canProceed={!!selected} loading={loading} onNext={handleContinue} onBack={isEditing ? () => router.back() : undefined}>
