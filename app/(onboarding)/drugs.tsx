...
export default function DrugsScreen() {
  ...
  useEffect(() => {
    if (currentUser && !hasLoadedData) {
      if (currentUser.drugs) setSelected(currentUser.drugs);
      if (currentUser.drugsVisible !== undefined) setVisible(currentUser.drugsVisible);
  ...
  const handleContinue = async () => {
    ...
    setLoading(true);
    try {
      await updateBasics({ clerkId: userId, drugs: selected, drugsVisible: visible });
      if (isEditing) {
        router.back();
      } else {
  ...
  return (
    <OnboardingScreen question="Do you use drugs?" canProceed={!!selected} loading={loading} onNext={handleContinue} onBack={isEditing ? () => router.back() : undefined}>
