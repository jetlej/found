...
// Test user photo URLs from Wikimedia Commons (public domain / CC licensed)
// Keyed by user NAME (not ID) so this survives DB resets.
// At runtime, names are resolved to user IDs via getTestUsers query.
...
// Main action: looks up user IDs by name, then schedules photo seeding per user
export const seedTestPhotos = action({
  ...
  handler: async (ctx) => {
    ...
    const names = Object.keys(USER_PHOTOS);
    let scheduled = 0;
